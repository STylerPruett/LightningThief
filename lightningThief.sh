...
nmapVersions() {
    ...
    if [[ -z "$ports" ]]; then
        echo "Error: No ports to scan. Did you run nmapOpen first?"
        return 1
    fi
...
enumerateSite()
{
    ...
    if [[ "$httpPorts" == "ALL" ]]; then
        for key in $(printf "%s\n" "${!http_service_dict[@]}" | sort -n); do
            gobuster -u http://$targetIP:$key -w $discoveryPath
        done
    elif [[ "$httpPorts" == "LATER" ]]; then
        while true; do
            read -p "Select HTTP/S Port(s): " temp_httpPorts
            if [[ -v http_service_dict["$temp_httpPorts"] ]]; then
                httpPorts="$temp_httpPorts"
                setEnv "HTTP_PORTS" "$httpPorts"
                gobuster -u http://$targetIP:$httpPorts -w $discoveryPath
                break
            else
                echo "Port is not running a known http service"
            fi
        done
    else #TODO add support for individual, ranges and list
        gobuster -u http://$targetIP:$httpPorts -w $discoveryPath
    fi
